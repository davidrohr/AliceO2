...
inline int GPUReconstructionCPU::runKernel(krnlSetup&& setup, Args&&... args)
{
  ...
  if (mProcessingSettings.debugLevel >= 3) {
    GPUInfo("Running kernel %s (Stream %d, Range %d/%d, Grid %d/%d) on %s", GetKernelName<S, I>(), stream, setup.y.start, setup.y.num, nBlocks, nThreads, cpuFallback == 2 ? "CPU (forced)" : cpuFallback ? "CPU (fallback)"
                                                                                                                                                                                                          : mDeviceName.c_str());
  }
