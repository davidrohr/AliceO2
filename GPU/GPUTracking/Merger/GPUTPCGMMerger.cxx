...
GPUd() void GPUTPCGMMerger::ResolveHitWeights2(int32_t nBlocks, int32_t nThreads, int32_t iBlock, int32_t iThread)
{
  ...
  for (int32_t i = get_global_id(0); i < NMergedTracks(); i += get_global_size(0)) {
    ...
    uint32_t nHits = 0;
    uint8_t sharedRowCandidates[GPUCA_ROW_COUNT];
    ...
    if (nSharedRowCandidates) {
      ...
      if (nShared) {
        ...
        const auto* candidates = &mClusterCandidates[i * GPUCA_ROW_COUNT * n];
        ...
        for (uint32_t j = 0; j < nShared; j++) {
          auto* cj = &mClusterCandidates[(i * GPUCA_ROW_COUNT + sharedRowCandidates[j]) * Param().rec.tpc.rebuildTrackInFitClusterCandidates];
    ...
    for (unsigned int j = 0; j < GPUCA_ROW_COUNT; j++) {
      const auto& candidates = &mClusterCandidates[(i * GPUCA_ROW_COUNT + j) * Param().rec.tpc.rebuildTrackInFitClusterCandidates];
      if (candidates[0].best > 0) {
